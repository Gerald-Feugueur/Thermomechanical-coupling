...
Rint_air=Rayon[-1,0] #Rayon intérieur air
Rext_air=50 #Rayon extérieur air
...
rho=6.45/1000000 #Masse volumique [kg/mm^3]
rho_air=1.225/1000000000 #Masse volumique air [kg/mm^3]
lamda=18/1000 #Conductivité thermique  [W/(mm*K)]
cp=322 #Capacité thermique [J/Kg*K]
cp_air=1005 #Capacité thermique air [J/Kg*K]
alpha=11/10**6 #Dilatation thermique [1/K]
q=0 #Flux de chaleur
E=28e3 #Module d'Young
nu=0.3 #Coefficient de Poisson
To=20+273 #Température homogène à l'intant initial air
Tf=70+273 #Température finale de chauffe air
P=0.85 #Pression
F_l=P*2*ma.pi*Rint/na #Force linéique appliqué aux noeuds
h_air=3/10**6 #Coefficient de convection thermique de l'air [W/mm^2*K]
Resistance=1/(h_air*2*ma.pi*Rext*23) #Résistance thermique[K/W]
PHI=rho_air*ma.pi*(Rext_air**2-Rint_air**2)*cp_air*(Tf-To) #Flux thermique [W/mm]
Tp=Tf-PHI*Resistance
kappa=Tp/Tf #Coefficient d'absorption 
...
def MAT_RIGI_MECA_ASSE_2D(): #Assemblage de la matrice de rigidité mécanique
    K=np.zeros(((nr+1)*(2*na),(nr+1)*(2*na)))
    for i in range(len(connect)):
        M=MAT_RIGI_MECA(int(coord[int(connect[i,0])-1,0]),int(coord[int(connect[i,0])-1,1]),int(coord[int(connect[i,1])-1,0]),int(coord[int(connect[i,1])-1,1]),int(coord[int(connect[i,2])-1,0]),int(coord[int(connect[i,2])-1,1]),int(coord[int(connect[i,3])-1,0]),int(coord[int(connect[i,3])-1,1]))
        loc=np.array(location[i,:],int)
        L=loc.tolist()
        for j in range(8):
            for k in range(8):
                K[L[k]-1,L[j]-1]+=M[k,j]
...
def MAT_RIGI_THER_ASSE_2D_DIAG(M):
...
wb = xlrd.open_workbook('T_air.xlsx')
 
# lecture des données dans chaque feuille

TEMP_AIR= wb.sheet_by_name(u'CHAMP_TEMP')

nb_ligne=TEMP_AIR.nrows
nb_colonne=TEMP_AIR.ncols

T_AIR=np.zeros((nb_ligne,nb_colonne))
for i in range(nb_colonne):
    T_AIR[:,i]=TEMP_AIR.col_values(i)

workbook = xlsxwriter.Workbook('T_bague.xlsx')
...
def CALC_CHAMP_TEMP(T0,C,K,F):
    ...
    for i in range(N_time):
        ...
        for k in N_ext:
            T_change=T
            for j in N_int:
                T_change[k-1,0]=(T_AIR[j-1,i+1]-To)*kappa+To
...
a.plot(x1,x2,label="Ring visualisation",color="r")
plt.scatter(coord[:,0],coord[:,1],label="Mesh nodes",s=5)
plt.axis('equal')
plt.axis([-1.2*Rext, 1.2*Rext, -1.2*Rext, 1.2*Rext])
plt.title("Mesh visualisation ("+str((nr+1)*na)+" Nodes and "+str(na*nr)+" Elements)",fontsize=16)
...
x1 = rext*np.cos(theta0)
x2 = rext*np.sin(theta0)
b.plot(x1,x2,label="Ring visualisation",color="r")
...
plt.title("Temperature evolution as a function of time",fontsize=16)
...
x1 = rext*np.cos(theta0)
x2 = rext*np.sin(theta0)
ax=plt.scatter(coord[:,0],coord[:,1],c=CHAMP_TEMP[:,0],cmap='jet',s=50,label="Nodes temperature")
plt.colorbar(ax)
...
x1 = rext*np.cos(theta0)
x2 = rext*np.sin(theta0)
ax=plt.scatter(coord[:,0],coord[:,1],c=CHAMP_TEMP[:,-1],cmap='jet',s=50,label="Nodes temperature")
plt.colorbar(ax)
...
x1 = rext*np.cos(theta0)
x2 = rext*np.sin(theta0)
ax=plt.scatter(coord[:,0],coord[:,1],c=CHAMP_TEMP[:,ma.floor(0.1*N_time)],cmap='jet',s=50,label="Nodes temperature")
plt.colorbar(ax)
...
x1 = rext*np.cos(theta0)
x2 = rext*np.sin(theta0)
ax=plt.scatter(coord[:,0],coord[:,1],c=CHAMP_TEMP[:,ma.floor(0.2*N_time)],cmap='jet',s=50,label="Nodes temperature")
plt.colorbar(ax)
#ax.set_clim(min(CHAMP_TEMP[:,ma.floor(2*N_time/3)]),max(CHAMP_TEMP[:,ma.floor(2*N_time/3)]))
ax.set_clim(To,Tf)
plt.axis('equal')
plt.axis([-1.2*Rext, 1.2*Rext, -1.2*Rext, 1.2*Rext])
plt.title("t="+str(round(0.2*time,2))+"s")
plt.tick_params(labelsize=12)
...
x1 = rext*np.cos(theta0)
x2 = rext*np.sin(theta0)
ax=plt.scatter(coord[:,0],coord[:,1],c=CHAMP_TEMP[:,ma.floor(0.3*N_time)],cmap='jet',s=50,label="Nodes temperature")
plt.colorbar(ax)
#ax.set_clim(min(CHAMP_TEMP[:,ma.floor(2*N_time/3)]),max(CHAMP_TEMP[:,ma.floor(2*N_time/3)]))
ax.set_clim(To,Tf)
plt.axis('equal')
...
x1 = rext*np.cos(theta0)
x2 = rext*np.sin(theta0)
ax=plt.scatter(coord[:,0],coord[:,1],c=CHAMP_TEMP[:,ma.floor(0.5*N_time)],cmap='jet',s=50,label="Nodes temperature")
plt.colorbar(ax)
#ax.set_clim(min(CHAMP_TEMP[:,ma.floor(2*N_time/3)]),max(CHAMP_TEMP[:,ma.floor(2*N_time/3)]))
ax.set_clim(To,Tf)
plt.axis('equal')
plt.axis([-1.2*Rext, 1.2*Rext, -1.2*Rext, 1.2*Rext])
plt.title("t="+str(round(0.5*time,2))+"s")
plt.xlabel("$x (mm)$",style='italic',fontsize=16)
plt.tick_params(labelsize=12)
...
py.offline.plot(fig,filename='Champ_de_temperature_SMA.html')
